...
export default function DeleteModal({ open, onClose, onDelete, isDeleting = false }) {
    ...
    return (
        <Modal open={open} onClose={onClose} maxWidth="xs" fullWidth padding={4}>
            <Box sx={containerStyles}>
                <Box sx={iconContainerStyles}>
                    <ErrorOutlineIcon color="error.dark" />
                </Box>
                <Typography variant="h1" color="black.100" sx={{ my: 3 }}>
                    Borrar Registro
                </Typography>
                ...
                <Grid container sx={{ width: '100%' }} spacing={1}>
                    <Grid size={6}>
                        <Button kind="secondary" fullWidth onClick={onClose}>
                            Cancelar
                        </Button>
                    </Grid>
                    <Grid size={6}>
